...
import { Layout, Row, Table, Input, Col, Form } from 'antd';
import Button from 'components/Button';
import { FORM_FIELDS } from 'constants/formFields';
import { listManagers } from 'reducers/managers';
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: '',
        render: (_, row) => {
          const func = find(propEq('function', 'CARD_MANAGEMENT'))(
            row.managerFunctions,
          );
      ...
      },
      {
        ...
        dataIndex: '',
        render: (_, row) => {
          const func = find(propEq('function', 'MEMBER_MANAGEMENT'))(
            row.managerFunctions,
          );
          return <span>{this.getPermission(func)}</span>;
        },
      },
      {
        ...
        dataIndex: '',
        render: (_, row) => {
          const func = find(propEq('function', 'DEVICE_MANAGEMENT'))(
            row.managerFunctions,
          );
          return <span>{this.getPermission(func)}</span>;
        },
      },
      {
        ...
        dataIndex: '',
        render: (_, row) => {
          const func = find(propEq('function', 'SYSTEM_MANAGEMENT'))(
            row.managerFunctions,
          );
          return <span>{this.getPermission(func)}</span>;
        },
      },
      {
        ...
        dataIndex: '',
        render: (_, row) => {
          const func = find(propEq('function', 'ASSIST_FINDING_MANAGEMENT'))(
            row.managerFunctions,
          );
          return <span>{this.getPermission(func)}</span>;
        },
        ...
        render: (_, manager) => {
          return (
            <Button
              ...
              onClick={() => this.handleEditModalVisible(manager)}
            />
          );
        },
      },
        ...
        <Content style={{ padding: '0px 24px 0px 8px' }}>
          ...
          <Row style={{ marginBottom: 15 }}>
            ...
            <Col span={8} style={{ textAlign: 'right' }}>
              <Button
                actionTypes={FORM_FIELDS.EDITABLE}
                onClick={() => this.handleEditModalVisible()}>
                新增管理者
              </Button>
            </Col>
          </Row>
